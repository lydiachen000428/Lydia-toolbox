...
def build_packing_list(config_path):
    ...
    po           = cfg['po']
    _today_raw   = cfg.get('date') or str(date.today())   # 空字串也退回今天
    try:
        ...
        today = _today_raw
    payment_term = cfg.get('payment_term') or 'T/T NET ETD 75DAYS'
    # 多行地址合併成單行，寫入 PAC A6（空值則保留模板原地址）
    ship_to = ', '.join(
        ln.strip() for ln in cfg.get('ship_to', '').splitlines() if ln.strip())
    items        = cfg['items']
    n            = len(items)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Build SCOSCHE Packing List Excel')
    parser.add_argument('--config', required=True, help='Path to JSON config file')
    args = parser.parse_args()
    result = build_packing_list(args.config)
